...
def get_elevetion(latitude, longitude):
    ...
    response = urllib.request.urlopen(
        'https://maps.googleapis.com/maps/api/elevation/json?locations=' + str(latitude) + ',' + str(
            longitude) + '&key=AIzaSyDI03ffKcFoSS91qoiIZkp69gohHqGV66o')
...
def next_coordinate(latitude, longitude, distance=0.5, bearing=0):
    ...
    nextlatitude = math.asin(
        math.sin(latitude) * math.cos(distance / 3960) + math.cos(latitude) * math.sin(distance / 3960) * math.cos(
            bearing))
    nextlongitude = longitude + math.atan2(math.sin(bearing) * math.sin(distance / 3960) * math.cos(latitude),
                                           math.cos(distance / 3960) - math.sin(latitude) * math.sin(nextlatitude))
